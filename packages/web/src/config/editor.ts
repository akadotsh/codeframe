...
export const languageConfig = {
  ...
  "C#": defineLanguage("cs", "csharp", `Console.WriteLine("Hello from Code Frame");`),
  Clojure: defineLanguage("clj", "clojure", `(println "Hello from Code Frame")`),
  Console: defineLanguage("txt", "console", `$ npm run build\n✓ built successfully`),
  Crystal: defineLanguage("cr", "crystal", `message = "Hello from Code Frame"\nputs message`),
  CSS: defineLanguage("css", "css", `.code-frame {\n  display: grid;\n  border-radius: 24px;\n}`),
  Cypher: defineLanguage("cql", "cypher", `MATCH (user:User)\nRETURN user.name`),
  Dart: defineLanguage("dart", "dart", `void main() {\n  print('Hello from Code Frame');\n}`),
  ...
  Elm: defineLanguage("elm", "elm", `greeting : String\ngreeting =\n    "Hello from Code Frame"`),
  ERB: defineLanguage("erb", "erb", `<h1><%= @title %></h1>`),
  Elixir: defineLanguage("ex", "elixir", `message = "Hello from Code Frame"\nIO.puts(message)`),
  Erlang: defineLanguage("erl", "erlang", `hello() ->\n    io:format("Hello from Code Frame~n").`),
  Gleam: defineLanguage(
    "gleam",
    "gleam",
    `pub fn main() {\n  io.println("Hello from Code Frame")\n}`,
  ),
  ...
  Haskell: defineLanguage(
    "hs",
    "haskell",
    `main :: IO ()\nmain = putStrLn "Hello from Code Frame"`,
  ),
  HTML: defineLanguage("html", "html", `<main>\n  <h1>Hello from Code Frame</h1>\n</main>`),
  Java: defineLanguage(
    "java",
    "java",
  ...
  Julia: defineLanguage("jl", "julia", `message = "Hello from Code Frame"\nprintln(message)`),
  JSON: defineLanguage("json", "json", `{\n  "name": "Code Frame",\n  "ready": true\n}`),
  JSX: defineLanguage(
    "jsx",
    "jsx",
    `export function Greeting() {\n  return <h1>Hello from Code Frame</h1>;\n}`,
  ),
  Kotlin: defineLanguage("kt", "kotlin", `fun main() {\n  println("Hello from Code Frame")\n}`),
  ...
  Lisp: defineLanguage("lisp", "lisp", `(format t "Hello from Code Frame~%")`),
  Lua: defineLanguage("lua", "lua", `local message = "Hello from Code Frame"\nprint(message)`),
  Markdown: defineLanguage(
    "md",
    "markdown",
    `# Hello from Code Frame\n\nCreate beautiful code images.`,
  ),
  MATLAB: defineLanguage("m", "matlab", `message = "Hello from Code Frame";\ndisp(message);`),
  Move: defineLanguage(
    "move",
    "move",
    `module code_frame::hello {\n  public fun ready(): bool { true }\n}`,
  ),
  Nix: defineLanguage(
    "nix",
    "nix",
  ...
  Plaintext: defineLanguage("txt", "text", `Hello from Code Frame`),
  Powershell: defineLanguage(
    "ps1",
    "powershell",
    `$message = "Hello from Code Frame"\nWrite-Output $message`,
  ),
  "Objective-C": defineLanguage(
    "m",
    "objc",
    `NSString *message = @"Hello from Code Frame";\nNSLog(@"%@", message);`,
  ),
  OCaml: defineLanguage(
    "ml",
    "ocaml",
    `let message = "Hello from Code Frame";;\nprint_endline message;;`,
  ),
  ...
  R: defineLanguage("r", "r", `message <- "Hello from Code Frame"\nprint(message)`),
  Ruby: defineLanguage("rb", "ruby", `message = "Hello from Code Frame"\nputs message`),
  Rust: defineLanguage(
    "rs",
    "rust",
    `fn main() {\n    let status = "ready";\n    println!("Code Frame is {status}");\n}`,
  ),
  ...
  Swift: defineLanguage("swift", "swift", `let message = "Hello from Code Frame"\nprint(message)`),
  ...
  XML: defineLanguage("xml", "xml", `<code_frame>\n  <title>Hello</title>\n</code_frame>`),
  YAML: defineLanguage(
    "yaml",
    "yaml",
    `name: Code Frame\nfeatures:\n  - syntax highlighting\n  - image export`,
